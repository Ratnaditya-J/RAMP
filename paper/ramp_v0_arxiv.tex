...
proximity, GPT-OSS activation probes,
...
or escalation evidence, but they do not yet justify positive runtime blocking weight. The main
...
\caption{RAMP accumulates staged evidence. The frozen runtime core uses prompt and activation
...
\begin{equation}
  s_{\mathrm{RAMP}} = 0.25 s_p + 0.75 s_a + 0.00 s_e.
\end{equation}
...
weight steps and 0.01 threshold steps. The selection objective orders candidates by F1, accuracy,
recall, false-positive rate, and AUROC on the calibration split. The reported weights and threshold
are the modal prompt+activation configuration across splits: threshold 0.53 was selected in 6 of 30
splits, and the prompt 0.25 / activation 0.75 weight pair was selected in 7 of 30 splits. We treat
this as a stability-selected research operating point, not as a universal deployment cutoff.
...
Taxonomy/audit signal, runtime weight 0 & Useful semantic boundary signal, but no lift over
prompt+activation. \\
Activation probe & Linear probe over GPT-OSS layer-19 hidden states & Positive runtime signal &
Layer comparison and reviewed calibration made this the strongest internal signal. \\
...
and domain-conditioned benign contrast. The final interpretation is conservative: embeddings
separate broad semantic neighborhoods and are useful for taxonomy review, but their overlap with
hard benign near-neighbors prevents them from earning positive runtime weight in the current policy.
...
19 under a low-FPR rule. We then retrained and recalibrated against the expanded reviewed-label set,
including hard cases where activations previously failed. Layer-19 activation probability remained
the strongest internal signal and earned the largest runtime weight in the frozen policy.
...
deterministic split stability rather than hand tuning. The decisive comparison for the frozen policy
is prompt+activation versus prompt+embedding+activation.
...
increases mean false-positive rate, so embedding is assigned zero runtime weight in the current policy.}
\label{fig:inputside}
\end{figure}
...
The reviewed-label evidence supports prompt+activation as the runtime core. Activation receives
...
best-supported runtime core; embedding, output, and session signals add audit or escalation value
